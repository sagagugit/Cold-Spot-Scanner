...
def createInterfaceCSV(pdbsToAnalyze):
    ...
    with open(os.path.join(RESULTS_DIR, PARAMETERS_DIR, 'PerAtomASA.csv'), 'w') as asaPerAtom:
        with open(os.path.join(RESULTS_DIR, PARAMETERS_DIR, 'PerAtomDistance.csv'), 'w') as distancePerAtom:
            ...
            for pdbName in pdbs:
                if pdbName not in pdbsNamesToChains: continue
                pdb = PDBReader.readFile(os.path.join(PDBS_DIR, pdbName), pdbsNamesToChains[pdbName])
                try:
...
def get_electrohydro_dicts(pdbsNamesToChains, cursor):
    ...
    for pdbName, chains in pdbsNamesToChains.items():
        ...
        pdb_to_inter_interactions[pdbName] = electrostat.calcInterElectroHydrophobic(pdb, interfaceAtoms, depthDistances)
        pdb_to_intra_interactions[pdbName] = electrostat.calcIntraElectroHydrophobic(pdb, interfaceAtoms, depthDistances)
...
def get_pi_dicts(pdbsNamesToChains, cursor):
    ...
    for pdbName, chains in pdbsNamesToChains.items():
        ...
        interfaceAtoms = getInterfaceAtoms(cursor, pdb)
        depthDistances = residue_depth(pdb)
        pdb_to_pi_interactions[pdbName] = electrostat.find_charged_pi(pdb, interfaceAtoms, depthDistances)
...
def get_pi_dicts_subtract(pdbsNamesToChains, cursor):
    ...
    for pdbName, chains in pdbsNamesToChains.items():
        if pdbName not in pdbsNamesToChains: continue
        inter_pi_interactions = pdb_to_pi_interactions[pdbName]
...
def get_anionic_dicts(pdbsNamesToChains, cursor):
    ...
    for pdbName, chains in pdbsNamesToChains.items():
        ...
        interfaceAtoms = getInterfaceAtoms(cursor, pdb)
        depthDistances = residue_depth(pdb)
        pdb_to_anionic_interactions[pdbName] = electrostat.anion_aromatic(pdb, interfaceAtoms, depthDistances)
...
def get_anion_aromatic_subtract(pdbsNamesToChains, cursor):
    ...
    for pdbName, chains in pdbsNamesToChains.items():
        ...
        interfaceAtoms = getInterfaceAtoms(cursor, pdb)
        depthDistances = residue_depth(pdb)
        pdb_to_anion_aromatic_interactions[pdbName] = electrostat.find_charged_pi(pdb, interfaceAtoms, depthDistances)
    for pdbName, chains in pdbsNamesToChains.items():
        if pdbName not in pdbsNamesToChains: continue
        inter_anion_interactions = pdb_to_anion_aromatic_interactions[pdbName]
...
def get_unfavorable_interactions(pdbsNamesToChains, cursor):
    ...
    for pdbName, chains in pdbsNamesToChains.items():
        ...
        pdb_to_unfavorable_interactions[pdbName] = unfavorable_catt
...
def charge_positive_negative_subbstract(pdbsNamesToChains, cursor):
    ...
    for pdbName, chains in pdbsNamesToChains.items():
        if pdbName not in pdbsNamesToChains: continue
        positive_negative = pdb_to_positive_negative[pdbName]
...
def same_charge_interface(pdbsNamesToChains, cursor):
    ...
    for pdbName, chains in pdbsNamesToChains.items():
        ...
        pdb = PDBReader.readFile(pdb_path, chains)
        interfaceAtoms = getInterfaceAtoms(cursor, pdb)
        pdb_to_same_charge[pdbName]= electrostat.identifying_same_charge(pdb, interfaceAtoms)
...
def inserting_the_data(pdbsToAnalyzeWithChains):
    ...
    dataToInsert = []
    for pdbName, chains in pdbsNamesToChains.items():
        pdb = PDBReader.readFile(os.path.join(PDBS_DIR, '%s_FH.pdb' % pdbName), pdbsNamesToChains[pdbName])
        if chains is None:
...
def calcEnergyTerms(pdbsToAnalyze):
    ...
    if not os.path.exists(os.path.join(RESULTS_DIR, COLD_SPOTS_DIR, 'Cold_spots_residues_due_to_CH_interactions.csv')):
        ...
        for pdbName, chains in pdbsNamesToChains.items():
            if pdbName not in pdbsNamesToChains: continue
            unfavorable_interactions= unfavroable_interactions_dict[pdbName]                        
    ...
    for pdbName, chains  in pdbsNamesToChains.items():
        if pdbName not in pdbsNamesToChains: continue
        pdb_path = '%s_cavities.pdb' % pdbName
        ...
        if diffrent_files:  
            ...
            with open(os.path.join('{}Sepereated_clusters.pdb'.format(pdbName)), 'w') as cluster_file:
            ...
            with open(os.path.join(RESULTS_DIR, COLD_SPOTS_DIR, 'Cavities_with_the_complex.pdb'), mode='w') as open_file:
                parser = PDBParser( PERMISSIVE=1)
                pdb_io = PDBIO()
                structure1 = parser.get_structure(pdb.file, '{}Sepereated_clusters.pdb'.format(pdbName))
                structure2 = parser.get_structure(pdb.file, '%s_C.pdb' % pdbName)
    ...
    if not os.path.exists(os.path.join(RESULTS_DIR, COLD_SPOTS_DIR, 'Cold_spots_due_to_SC_interactions.csv')):
        ...
        with open(os.path.join(RESULTS_DIR, COLD_SPOTS_DIR, 'Cold_spots_due_to_SC_interactions.csv'), mode="w") as same_int:
            ...
            for pdbName, chains in pdbsNamesToChains.items():
                if pdbName not in pdbsNamesToChains: continue
                same_charge_pi_interactions = same_charge_interactions_dict[pdbName]
...
#            print('Identified charged residues which can act as cold spots due to SC interactions', file=same_int)
            ...
            os.remove(os.path.join(pdbName + "_cavities.pdb"))
            os.remove(os.path.join("a" + pdbName))
            os.remove(os.path.join("b" + pdbName))
        ...
        with open(os.path.join(RESULTS_DIR, PARAMETERS_DIR, 'atom_depths.csv'), 'w') as depth:
            ...
            for pdbName, chains in pdbsNamesToChains.items():
                ...
                for atom, distance in atomDistances:
                    print(','.join([pdbName, atom.chain, str(atom.resId), atom.residue, atom.symbol, str(distance)]), file=depth)
